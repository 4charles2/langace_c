...
int main(int argc, char **argv)
{
	...
	while(continuer)
	{
		...
		/*Mettre en pause le programme si la fenetre et réduite*/
		if((event.active.state & SDL_APPACTIVE) == SDL_APPACTIVE)
		{
			if(event.active.gain == 0)//La fenetre est réduite
				/*Mettre le programe en pause = 1;*/
			else if(event.active.gain == 1)//La fenetre est restauré
				/*pause = 0;*/
		}
